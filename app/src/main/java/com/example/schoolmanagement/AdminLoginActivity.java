...
public class AdminLoginActivity extends AppCompatActivity {
                ...
                if (task.isSuccessful()) {
                    ...
                    adminRef.addListenerForSingleValueEvent(new ValueEventListener() {
                        ...
                        public void onDataChange(@NonNull DataSnapshot snapshot) {
                            if (snapshot.exists() || email.equalsIgnoreCase("saurabhkalamkar07@gmail.com")) {
                                handleSuccessfulAdminLogin(email);
                            } else {
                                mAuth.signOut();
                                hideLoading(adminLoginBtn);
                                Toast.makeText(AdminLoginActivity.this, "Access Denied: You do not have Admin privileges", Toast.LENGTH_SHORT).show();
                            }
                        }
    ...
    private void handleSuccessfulAdminLogin(String email) {
        ...
        new Thread(() -> {
            try {
                ...
                runOnUiThread(() -> {
                    hideLoading(adminLoginBtn);
                    Toast.makeText(AdminLoginActivity.this, "OTP send failed: " + e.getMessage(), Toast.LENGTH_LONG).show();
                });
            }
        }).start();
    }
